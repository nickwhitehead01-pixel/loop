...
class TestLessonUploadEndpoint:
    ...
    async def test_upload_lesson_persists_record(
        self, async_db: AsyncSession, mock_ollama_embed, mock_ollama_generate_full
    ):
        """Test that uploading a lesson creates a persisted Lesson record immediately.

        Summary and precomputed features are generated by the background worker
        after the upload returns, so they are not asserted here.
        """
        ...
        # Summary is generated by the background worker — not synchronously on upload
