...
class Lesson(Base):
    ...
    # Pre-computed live-lesson features. Generated by the background worker
    # once after upload, then read (not regenerated) every time the lesson is
    # played live. Both fields are nullable — null means "not generated yet"
    # or "all retries exhausted" — and the matcher service degrades to no-op
    ...
    #
    # prompt_cards shape:
    #   [{"id": str, "triggers": [str, ...], "question": str,
    #     "color": str, "trigger_embedding": [float, ...]}, ...]
    #
    # The embedding is stored inline (per-lesson card counts are small
    # enough that this is cheaper than a separate ChromaDB collection).
    glossary: Mapped[list | None] = mapped_column(JSON, nullable=True)
    prompt_cards: Mapped[list | None] = mapped_column(JSON, nullable=True)
    precomputed_features_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    # Increment on every failed precompute attempt. The worker skips lessons
    # that have hit the cap, so a permanently-broken upload (e.g. corrupted
    # PDF, Gemma OOM on a specific document) doesn't loop forever.
    precomputed_features_attempts: Mapped[int] = mapped_column(
